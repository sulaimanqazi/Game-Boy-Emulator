...
u16 reverse(u16 n){ //So that we can return AF BC DE HL
    //takes a u16 and reverses the order of its two bit halves
    //and performs an OR between the two results t
    return ((n & 0xFF00) >> 8) | ((n & 0x00FF) << 8);
}

u16 cpu_read_reg(reg_type rt){
    switch(rt) { //switch through register type passed in return the register
...
//Taking a register type and a value , switching for each of the types and setting the ctx.regs value

void cpu_set_reg(reg_type rt, u16 val) {
    switch(rt) {
        //8 bit values get ANDed with 0xFF
...
cpu_registers *cpu_get_regs() {
    return &ctx.regs; //Return cpu registers
}
