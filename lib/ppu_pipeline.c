...
u32 fetch_sprite_pixels(int bit, u32 colour, u8 bg_colour) {
    for(int i = 0; i < ppu_get_context()->fetched_entry_count; i++) {
        ...
        if (!bg_priority|| bg_colour == 0 ) {
            colour = (ppu_get_context() -> fetched_entries[i].f_pn) ? lcd_get_context()->sp2_colours[hi|lo] : lcd_get_context()-> sp1_colours[hi|lo];
...
bool pipeline_fifo_add(){
    ...
    for(int i = 0; i < 8; i++){
        ...
        u32 colour = lcd_get_context()->bg_colours[hi | lo];
...
void pipeline_load_window_tile() {
    ...
    if(ppu_get_context()-> pfc.fetch_x+7 >= lcd_get_context()->win_x && ppu_get_context()->pfc.fetch_x+7 <lcd_get_context()->win_x +YRES+14) {
        if(lcd_get_context()->ly >= window_y && lcd_get_context()->ly < window_y + XRES) {
            ...
            ppu_get_context()->pfc.bgw_fetch_data[0] = bus_read(LCDC_WIN_MAP_AREA + ((ppu_get_context()->pfc.fetch_x + 7 - lcd_get_context()->win_x) / 8) + (w_tile_y * 32));
